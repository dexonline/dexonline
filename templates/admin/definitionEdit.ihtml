<form action="definitionEdit.php{if $definitionId}?definitionId={$definitionId}{/if}" method="post" name="frmDef">
  ...
  <table class="editableFields">
    {foreach from=$homonyms item=h key=i}
      <tr>
        ...
        <td>
          {include file="bits/lexemName.ihtml" lexem=$h}
  ...
  <table class="editableFields">
    ...
    <tr>
      ...
      <td>
        {if $source->canModerate}
          {include file="sourceDropDown.ihtml" sources=$allModeratorSources src_selected=$def->sourceId skipAnySource=true}
        {else}
    ...
    <tr>
      <td>Starea:</td>
      <td>
        {include file="statusDropDown.ihtml" name="status" statuses=$allStatuses selectedStatus=$def->status}
