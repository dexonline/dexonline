...
{block "content"}
  ...
  <form id="claimIdForm" method="post" action="{$wwwRoot}auth/revendicare">
    <input type="radio" name="loginType" value="0" id="claimAccount" {if $loginType == 0}checked{/if}>
    ...
    {if isset($data.fullname)}
      <input type="radio" name="loginType" value="1" id="useFullName" {if $loginType == 1}checked{/if}>
      <label for="useFullName">Numele întreg ({$data.fullname})</label><br>
    {/if}

    {if isset($data.nickname)}
      <input type="radio" name="loginType" value="2" id="useNickname" {if $loginType == 2}checked{/if}>
      <label for="useNickname">Porecla OpenID ({$data.nickname})</label><br>
    {/if}

    <input type="radio" name="loginType" value="3" id="chooseNickname" {if $loginType == 3}checked{/if}>
