...
{block "content"}
  ...
  <div class="card mb-3">
    <div class="card-header fs-2 px-2 py-0 d-flex justify-content-between">
      {if isset($prevmon)}
        <a title="{t}previous{/t}" href="{Router::link('proverb/view')}/{$prevmon}">
          {include "bits/icon.tpl" i=chevron_left}
        </a>
      {/if}
      {if isset($nextmon)}
        <a title="{t}next{/t}" href="{Router::link('proverb/view')}/{$nextmon}">
  ...
  <h3>Dicționarul vizual de proverbe</h3>
  <div class="card mb-4">
              ...
              <td class="activeMonth">
                <div class="thumb">
                  {if $day.wotd && $day.wotd->image && $day.visible}
                    <a href="{Router::link('proverb/view')}/{$day.wotd->id}">
                      <img
