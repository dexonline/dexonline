...
if ($count > $MAX_AFFECTED) {
  if ($realRun) {
    session_setFlash("{$count} definiții se potrivesc, numai {$MAX_AFFECTED} au fost modificate.", 'info');
  } else {
    session_setFlash("{$count} definiții se potrivesc, maxim {$MAX_AFFECTED} vor fi modificate.");
  }
}
